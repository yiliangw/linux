...
static bool cxlsim_mem_on;

static __init int parse_cxlsim_mem_on(char *arg)
{
    cxlsim_mem_on = true;
    return 0;
}
early_param("cxlsim_mem_on", parse_cxlsim_mem_on);
...
static void __init numa_init_array(void)
{
	...
	for (i = 0; i < nr_cpu_ids; i++) {
		if (early_cpu_to_node(i) != NUMA_NO_NODE)
			continue;
		numa_set_node(i, rr);
		// Prevent assigning cpus to the CXL node
		if (cxlsim_mem_on) {
...
static int __init dummy_numa_init(void)
{
	...
	if (cxlsim_mem_on) {
		cxl_mem_start = e820_table->entries[e820_table->nr_entries-1].addr;
		cxl_mem_size = e820_table->entries[e820_table->nr_entries-1].size;
		cxl_mem_end = cxl_mem_start + cxl_mem_size;
		...
		printk(KERN_INFO "cxlsim: cxl_mem_start=%#018Lx, cxl_mem_size=%#018Lx, cxl_mem_end=%#018Lx\n, dram_start=%#018Lx, dram_end=%#018Lx\n",
